...
# 7) Eğitim (opsiyonel) + --video-paths entegrasyonu
if [[ "${SKIP_TRAIN:-0}" -ne 1 ]]; then
  ...
  # --video-paths hazırlığı (env: VIDEO_HINTS="path1 path2 ...")
  if [[ -n "${VIDEO_HINTS:-}" ]]; then
    read -r -a _H <<< "$VIDEO_HINTS"
  else
    _H=(/home/at2438/SLEAP_FILES/VIDEOS /home/at2438/SLEAP_FILES)
  fi
  VP=(); for p in "${_H[@]}"; do VP+=(--video-paths "$p"); done

  # Eğitim (centroid -> centered_instance)  [ENTEGRE EDİLDİ]
